import { ARGB_GREEN, ARGB_LIGHT_GRAY, ARGB_RED, ARGB_WHITE, ARGB_YELLOW, NUMBER_FIGHTS_POOLS, POOLS_BRACKETS_COLS, POOLS_TEAM_ROW_START, POOL_BRACKET_HEADER_VALUES, POOL_BRACKET_SUB_HEADER_VALUES, getFont } from './sheetConstants.js'
...
const populatePoolsBracketsSheet = (sheet, { teams }) => {
  ...
  sheet.getColumn('T').alignment = { vertical: 'middle', horizontal: 'left', wrapText: false };

  const cellR2 = sheet.getCell('R2');
  cellR2.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ARGB_YELLOW } };
  
  const cellS2 = sheet.getCell('S2');
  cellS2.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ARGB_RED } };
  ...
  for (let i = 3; i <= NUMBER_FIGHTS_POOLS + POOLS_TEAM_ROW_START; i++) {
